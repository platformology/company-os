# The engine — the seats, written the way a runtime expects them
...
[runtime-openclaw.md](runtime-openclaw.md) already did the reading: it took [openclaw/openclaw](https://github.com/openclaw/openclaw) — a public MIT-licensed personal-assistant runtime, developed in the open by the OpenClaw Foundation — and held it up against our seats, and the finding was that we had been describing its shape by hand for weeks. This page takes the one step after that. It writes the files that shape actually expects, one folder per seat, so that whoever is standing at the M1 has **something to hand over** instead of a description of something to hand over.

No runtime got designed here. Every file layout, file name, and loading rule below is OpenClaw's, documented and public. What is ours is the seats and the words in them, and those were already written in [seats/](seats/README.md), [bodies/](bodies/README.md), and [desks/](desks/README.md).

The original twenty-three workspace folders were written on the engine branch. **This branch adds twenty-three extra unassigned craft workspaces** — the seats Michael named on 20 Aug 2026. They exist, they are still unassigned, they still have no aliases, and they still do not send. A folder here does not staff a seat.
...
handbook/engine/workspaces/<slug>/          one folder per seat that has a workspace on this branch
...
The original twenty-three slugs are already on [desks/README.md](desks/README.md) — `pa`, `coo`, `cmo`, `head-of-content`, `head-of-demand`, `cso`, `head-of-sales`, `sdr`, `cfo`, `controller`, `bookkeeper`, `cto`, `head-of-engineering`, `developer`, `cco`, `head-of-author-success`, `client-success`, `chro`, `people-ops`, `vp-campaigns`, `campaign-coordinator`, `vp-retreats`, `retreat-producer`. Those workspace folders were written on the engine branch. Same slug as the seat page, the body, the desk, and the Ollama alias. One slug, five places, no translation table.

**The twenty-three extra unassigned craft slugs on this branch:** `senior-digital-designer`, `junior-digital-designer`, `senior-print-designer`, `junior-print-designer`, `social-media-strategist`, `social-account-expert`, `book-strategist`, `book-writer`, `book-conceptual-editor`, `book-copy-writer`, `book-technical-editor`, `book-reader`, `persona-architect`, `strategist`, `copy-writer`, `sales-writer`, `copy-editor`, `sales-editor`, `seo-expert`, `pr-media`, `speaking-universities`, `speaking-associations`, `pendulum-wiki`. Same slug as the seat page, the body, the desk, and the router. **No Ollama alias.** [Persona Architect](seats/persona-architect.md) is empty Craft, not Anthony — his recorded title stays on [coo.md](seats/coo.md). A workspace here does not fill any of these seats, name a social account, invent a speaking date, claim a wiki exists, or invent a client.
...
| `IDENTITY.md` | The seat's name, its slug, its status, and who fills it — no alias on the extra twenty-three | The seat page — [seats/](seats/README.md), and [people.md](people.md) for the holder |
...
- **Michael Drew** is who every seat works for. Nothing reaches a client, a prospect, or the public without his sign-off.
...
- **Anthony C. Garcia — COO / Chief Persona Architect — is the one recorded seat holder.** His `IDENTITY.md` on the engine branch says so. Every other seat, including all twenty-three extra craft seats and including [Persona Architect](seats/persona-architect.md), says **"not recorded,"** and they keep saying it until Michael records someone. A workspace existing for a seat does not fill the seat.
...
1. **One workspace per seat.** OpenClaw is blunt that two agents must never share a state directory — their auth and their sessions collide. Ours is one folder per seat that never touches another.
...
- **No per-seat model.** One hand, twenty-three original bodies. The extra twenty-three get no alias.
...
1. **Copy each `engine/workspaces/<slug>/` to that seat's workspace directory on the Mac, verbatim.** OpenClaw's default is `~/.openclaw/workspace` for the first agent, and each additional agent gets its own — either set per-agent in config, or it resolves to a `workspace-<agentId>` folder of its own. One folder, one directory, no sharing.
2. **Turn bootstrap seeding off** so OpenClaw does not write its own starter files over ours.
3. **Build the twenty-three original Ollama aliases**, `FROM office-hand`, one `SYSTEM` paste per original seat, exactly as [desks/BUILD.md](desks/BUILD.md) already spells out. `FROM` never changes. Twenty-three names cost one download. **Do not build aliases for the extra twenty-three craft seats.** An unassigned seat does not get a model.

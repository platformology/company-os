...
| | |
|---|---|
| **This pass covers** | What the seat page records, who the clients are per the 18 Aug 2026 extract with that date attached, why there is no score, and what is not recorded — said as "not recorded" |
| **This pass refuses** | Any lead, lead list, lead count, inbound volume, conversion rate, score, A/B/C tier, or MQL/SQL cutoff. There is no criterion, weight, or threshold to score against, and a plan is not the place one gets invented |

## The SOP that runs it

The seat page opens [Buyer data room](https://app.notion.com/p/3c025e30d68b8135bdcdc9010e649855) (Notion) — what a diligence reader already sees. **[seat-job-map.md](../seat-job-map.md) points this row at a different page, the [Campaign lead scoring draft](https://app.notion.com/p/3c125e30d68b81f89125cfbbe398c770)**; the desk resolves that by following the seat page, and it is recorded in [desks/CONFLICTS.md](../desks/CONFLICTS.md). That is not a fight, and it is not a reason to invent a lead list.

**There is no scoring model in the corpus, and five fields on the campaign lead scoring draft are unnamed** ([SOURCES.md](../../SOURCES.md)). [campaign-lead-scoring](../../skills/campaign-lead-scoring/SKILL.md) already refuses to invent the model, and this seat does not get a different answer. Do not fill the five fields, and do not build a model to make the draft usable.

The client facts this seat works against are settled on [clients.md](../clients.md) and do not get re-derived:

- **18 Aug 2026 extract: 1 Alumni, 67 Past, 0 Current, 0 Prospect.** Quote it with that date; it was true on the extract date, not forever.
- **Cornelia Choe (The Leaders Alliance) is the 1 Alumni** — her meeting is **support, not a pitch**, and her **two Zoom URLs are unresolved. Do not pick one.**
- **Hoffman is a question, not a Current client.**
- **Folder names are not clients.** A folder existing for a name is not evidence of anything.
- **A link to the Notion Clients database itself is not recorded in this repo** ([SOURCES.md](../../SOURCES.md)).

## The KPI this plan is pointed at

| | |
|---|---|
| **KPI** | **Consults booked.** Named on the [KPIs](https://app.notion.com/p/b104782240524af595577abaef8dc928) database (Notion), where the Owner field reads SDR. Copied here as a name, nothing more — this repo does not restate the dashboard, and this plan does not rename, drop, or measure it. |
...
7. Asked to keep the CRM honest? Work in Notion Clients and change nothing that is settled: **Hoffman stays a question**, folder names stay not-clients, and moving anyone to Current or Prospect is an ask-Michael-first item on [locks.md](../locks.md).
8. Is it about Cornelia Choe? **Support, not a pitch**, and **both Zoom URLs go to Michael unresolved** — picking one is an ask-Michael-first item.
9. Asked for outreach? Draft only, from something real, and it does not leave the building. A price on a qualifying call comes off [offers.md](../offers.md) — both numbers where the ladder shows two, **no public price for Platinum Major List.**
10. Say where you looked, then run [reviews/last-gate-checklist.md](../../reviews/last-gate-checklist.md) and stop.
...
- Who fills this seat.
- Any lead, lead list, lead count, or score.
- No scoring model in the corpus, and five unnamed fields on the campaign lead scoring draft.
- No consult record and no qualification bar behind **Consults booked.**
...
- **Never invent a lead, a lead list, a lead count, or an inbound volume.**
- **Never assign a score, a tier, or a qualification threshold** — there is no model to score against.
- Never build a prospect list out of folder names or out of the 67 Past clients.
- Never record a client as Current or Prospect, and never move Hoffman off "question," to make the CRM look busier.
- Never treat HubSpot as the CRM or as a live system of record. It is closed.
- Never turn Cornelia Choe's Alumni support meeting into a pitch, and never pick between her two Zoom URLs.
- Never staff this seat or name a holder for it, and never let the CSO or Head of Sales pick its live work up.
...
- **Never put a figure in a KPI Current field, including a 0.** The 18 Aug 2026 client count is a separate fact and travels with its date — it is not a consult count.
- Never send. **No send without Michael.** No program GitHub. **And never treat a lock this plan does not happen to list as lifted** — [locks.md](../locks.md) is the list, and a plan is not.

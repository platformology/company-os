# VP Retreats

Shape: Holacracy Role — [holacracyone/Holacracy-Constitution](https://github.com/holacracyone/Holacracy-Constitution), `Holacracy-Constitution.md`. Seat tracked separately from seat holder — [JGalego/TeamAPI](https://github.com/JGalego/TeamAPI), `docs/spec/teamapi-extended-v1.md`. Not Retreat Producer — two separate seats; see [retreat-producer.md](retreat-producer.md).

**Purpose:** Own how a live Book Retreat or Platform in a Box engagement runs, start to finish.

**Owns:**
- [Retreat SOP](https://app.notion.com/p/3c025e30d68b8185b9a9f19dd56389e5) (Notion) — this repo does not restate the SOP.
- [PIAB SOP](https://app.notion.com/p/3c125e30d68b81d5beecda81a111e6b3) (Notion) — same.
- The [blank guest intake draft](https://app.notion.com/p/3c125e30d68b812486c8eac90777658c) (Notion) — whichever fields are blank there stay named missing; this repo does not fill them in from a guess.

**Reports into this seat:** [Retreat Producer](retreat-producer.md).

**Seat holder:** Not recorded — see [people.md](../people.md).

**Run this:** [run-vp-retreats](../../skills/run-vp-retreats/SKILL.md) — the router for this seat, for ownership questions — then [run-retreat](../../skills/run-retreat/SKILL.md) or [run-piab](../../skills/run-piab/SKILL.md) for a live engagement, and [blank-guest-intake](../../skills/blank-guest-intake/SKILL.md) for the intake draft. Every skill in the repo: [skills/README.md](../../skills/README.md).

**Open this in Notion:** [Retreat SOP](https://app.notion.com/p/3c025e30d68b8185b9a9f19dd56389e5) · [PIAB SOP](https://app.notion.com/p/3c125e30d68b81d5beecda81a111e6b3) · [Guest intake draft (blank)](https://app.notion.com/p/3c125e30d68b812486c8eac90777658c)

**Locked:** No send without Michael. No program GitHub. Prices quoted only from [offers.md](../offers.md). See [locks.md](../locks.md).

**Named missing:** who currently fills this seat; whichever fields are blank in the guest intake draft (see [SOURCES.md](../../SOURCES.md)).

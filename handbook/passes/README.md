...
The assigned seats with a job to run come first, in the order they sit in on [plans/README.md](../plans/README.md): PA is the first of them, COO is the second, CMO is the third. The rest have no pass page yet. That is a gap in this folder and nothing else — a missing row here does not mean a seat has no job, no plan, or no router. Which seats are assigned with a job to run is the seventeen-row table on that page, and this page does not copy it.
...
Named gaps stay named. Where a page here says a thing is not recorded — who fills the PA seat, the COO's accountabilities beyond the public title, the membership price and the lead magnet's absent Offers row on the CMO's — that is the answer until Michael sources one. Full list: [SOURCES.md](../../SOURCES.md).

...
| [ollama/ollama](https://github.com/ollama/ollama), `docs/modelfile.mdx` | The anatomy only: a model definition is a base it starts from plus a `SYSTEM` brief, so a named model can be made without touching the weights → the per-seat table on `handbook/bodies/README.md`. Its commands were deliberately **not** taken: no `ollama create`, no run line, no `PARAMETER` or sampler values on that page, because writing one out before Michael's go is Phase 4 taken early and a local model is locked until then (`handbook/locks.md`) |
...
| [adr/madr](https://github.com/adr/madr), `template/adr-template.md` | The `status:` field, where a decision can be recorded and accepted without being implemented, plus the "More Information" element that says *when and how* a decision should be realized rather than realizing it. That is the shape of the hand section on `handbook/bodies/README.md`: the hand is named, its status is "not installed," and when it gets installed belongs to Michael's go packet. Its considered-options, pros-and-cons, and numbered decision-log layout were left behind — this is one section on a run page, not a decision record |

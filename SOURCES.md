...
| [ollama/ollama](https://github.com/ollama/ollama), `docs/modelfile.mdx` | `FROM`, `PARAMETER num_ctx`, `SYSTEM """…"""`, and `ollama create <name> -f ./Modelfile` — the recipe on `handbook/bodies/README.md` that turns a body into a seat model without touching the weights. The same docs put `think` on the call rather than in the file, which is why that page shows it as `--think=false` on the command and not as a line in the Modelfile |
| [sammcj/llm-templates](https://github.com/sammcj/llm-templates), `Modelfile-qwen3` | Many named models built from one base, with the `ollama create …` line kept as a comment at the top of the Modelfile so whoever opens it can see how it was built → the example on `handbook/bodies/README.md`. Its GGUF paths, sampler tuning, and per-tool variants were left behind |
| [openclaw/openclaw](https://github.com/openclaw/openclaw), `README.md` plus `docs/concepts/agent.md`, `docs/concepts/agent-workspace.md`, `docs/concepts/multi-agent.md`, `docs/concepts/soul.md`, `docs/concepts/model-providers.md`, `docs/tools/skills.md`, `docs/tools/creating-skills.md` | The shape of a seat that actually works: one workspace per agent, a small fixed set of injected operating files (`AGENTS.md` for rules, `SOUL.md` for voice), skills as folders holding a `SKILL.md` with `name` + `description`, a per-agent allowlist that **replaces** the default rather than merging into it, a "missing file" marker instead of a quiet skip, and one local Ollama server shared by many agents rather than a model per agent → `handbook/runtime-openclaw.md`. Read and deliberately **not** adapted: its channels, bindings, pairing, streaming, agent-to-agent messaging, and registry installs — all of them are send paths, and `handbook/locks.md` says no send without Michael. Its gateway, installer, sandboxes, and companion apps were left behind; nothing in this repo installs it |

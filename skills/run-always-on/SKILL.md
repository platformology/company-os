---
name: run-always-on
description: Run the COO cycle Michael already locked — planning → meetings → execution → optimizing → next sourced slice — one report at a time, never fanned out, never inventing work. Use when a Grok Bot seat is picking up its next unit of work, or someone asks "what's the COO cycle" or "what runs 24/7 here."
---

# Run always-on

Reused from: [dswh/company-os](https://github.com/dswh/company-os), `departments/_canvas-template.md` (the loop shape, already the source for [handbook/weekday-loop.md](../../handbook/weekday-loop.md) and [handbook/cheap-loop.md](../../handbook/cheap-loop.md)), and [Oruga420/claude-code-skills](https://github.com/Oruga420/claude-code-skills), `loop-builder/SKILL.md` ("simple loop → ONE agent, start to finish — no handoffs, no extra layers: the whole task fits in one head") for the one-report-at-a-time, no-fan-out rule below.

## The cycle — locked by Michael, do not redesign it

Planning → Meetings → Execution → Optimizing → next sourced slice. Five steps, in that order, on repeat. This skill does not add a step, skip one, or reorder them.

## Bots 24/7. Humans not.

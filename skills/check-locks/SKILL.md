...
- **Start on the M5**, or on anything that assumes a machine, a VM, a cluster, or a local model already exists.
...
- **Never treat the M5 as available** until the machine is physically in hand, and never plan work that assumes a VM, a GKE cluster, or a local LLM exists. See [handbook/locks.md](../../handbook/locks.md).
